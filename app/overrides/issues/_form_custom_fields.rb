...
<%= if value.custom_field.commentable?
  self.text_field_tag("issue[custom_field_values]["+value.custom_field.id.to_s+"-comment]", value.comment, {:id => "issue_custom_field_comments_"+value.custom_field.id.to_s, placeholder: 'Ajouter un commentaire', class: 'issue_custom_field_comments'})
end %>
EOS
